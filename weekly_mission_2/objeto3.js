// Ejemplo 3: Objeto con diferentes propiedades
const myObject3 = {
    name: "Tulio",
    age: 2,
    nicknames: [
     "Tulipan",
      "Tulancingo",
      "Tulish"
    ],
    address: {
      zip_code: "10000",
      street: "Dr. Vertiz 11 Benito Juarez",
      city: "CDMX"
    }
  }
  console.log("Ejemplo 3: Objeto con diferentes propiedades")
  console.log(myObject3)
  console.log(myObject3.name)
  console.log(myObject3["nicknames"])

  //Nuevo objeto con propiedades diferentes

  const persona = {
    nombre: 'Luis Diaz',
    edad: 23,
    hobbies:[ //Lista de propiedades
        "Jugar videojuegos",
        "Hacer ejercicio",
        "Jugar futbol"
    ],
    direccion:{//nuevo objeto dentro de otro
        calle: "Boulevard 5 de mayo",
        colonia: "Barrio de San Antonio",
        CP: '72000'
    }
  }

  console.log(persona)